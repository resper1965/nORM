...
// Ensure type compatibility with OpenAI
...
export async function callAIGateway(
  ...
  const canProceed = await rateLimiter.checkLimit({
    key: rateLimitKey,
    limit: parseInt(process.env.MAX_OPENAI_REQUESTS_PER_HOUR || "100"),
    windowMs: 60 * 60 * 1000,
  });
    ...
    async () => {
      ...
      while (currentModel) {
        try {
          ...
          logger.error(`AI Gateway error with ${currentModel}`, error as Error);

          // Try fallback
          if (enableFallback && FALLBACK_CHAIN[currentModel]) {
            currentModel = FALLBACK_CHAIN[currentModel]!;
            logger.info("AI Gateway fallback", {
              from: model,
              to: currentModel,
            });
            continue;
